...
exportVars ()# ()
{
    ...
    # 2019/06/01 CJA - Was causing errors and I didn't use/need them 
...
stopProcess () #Stop this process and continue with the next
{
	if [ ${CONFIG[stopProcess]} -eq 0 ]; then
		log "Stopping process ${project}"
	fi
	CONFIG[stopProcess]=1
}
                ...
                if [ 0 -eq $? ]; then
                    ...
                    if [ 1 -eq $isSkipped ]; then
                        ...
                        if [ -f "$taskScript" ]; then
                                ...
                                passInArray=$?
                                if [ 0 -eq $passInArray -o '_none_' = "$project" ] && [ ${CONFIG[stopProcess]} -eq 0 ]; then 
				    ...
				    # 2019/05/29 CJA one line update
				    cd "$_home"
				    # at least make sure the correct directory 
				    # is in place b4 running a process.
                                    ...
                                    if [ "sh" = "$realTaskExtension" ]; then
                                        ...
                                        case "$realTaskExtension" in
                                            java)
                                                # note that your java code must declare no package 
                                                __vars=$(java "$task")
                                            ;;
                                            php)
                                            ...
                                            ;;
                                            py)
                                                __vars=$(python3 "$taskScript")
                                            ;;
                                            js)
                                                __vars=$(node "$taskScript")
                                            ;;
                                            *)
